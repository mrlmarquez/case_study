...
from langgraph.errors import (
    NodeInterrupt,
)  # interrupt for human-in-the-loop intervention
...
async def step_1(state, config: RunnableConfig):
    ...
    await adispatch_custom_event(
        "on_init_input", {"input": state["input"]}, config=config
    )
    return state
...
async def step_2(state, config: RunnableConfig):
    ...
    if len(state["input"]) > 5:
        ...
        await adispatch_custom_event(
            "on_waiting_user_resp", len(state["input"]), config=config
        )
        # Raise a NodeInterrupt to stop execution and request a shorter input (<= 5 characters)
        raise NodeInterrupt(
            f"Please provide an input </= than 5 characters, current is {state['input']}, with length {len(state['input'])}"
        )
    else:
        # Dispatch an event confirming that the input meets the length condition
        await adispatch_custom_event(
            "on_conditional_check", len(state["input"]), config=config
        )
...
async def step_3(state, config: RunnableConfig):
    ...
    await adispatch_custom_event(
        "on_complete_graph",
        {"input": state["input"], "len": len(state["input"])},
        config=config,
    )
    return state
